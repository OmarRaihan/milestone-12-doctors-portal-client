...
const BookingModal = ({ date, treatment, setTreatment }) => {
  ...
  return (
    <div>
      ...
      <div class="modal modal-bottom sm:modal-middle p-6">
        <div class="modal-box">
          <label for="booking-modal" class="btn btn-sm btn-circle absolute right-2 top-2">
            ✕
          </label>
          <h3 class="font-bold text-center text-lg text-teal-500">Booking For: {name}</h3>
          <form onSubmit={handleBooking} className="grid grid-cols-1 gap-2 justify-items-center mt-8">
            <input type="text" disabled value={format(date, "PP")} class="input input-bordered text-lg w-full max-w-xs" />
